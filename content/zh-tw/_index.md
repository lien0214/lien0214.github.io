...
Shopback 是東南亞最大的現金回饋平台，用戶數千萬。我在後端團隊負責 TypeScript monorepo，涉及客服基礎架構、Watchlist 功能和旅遊通知系統。

主要貢獻：
- **將 10+ 個跨服務 API 呼叫遷移為直接函式呼叫**，作為 monorepo 整合的一部分。驅動原因是容器負載不均——各自獨立的靜態資源容器造成流量尖峰和不穩定的自動擴展，因此合併進共用的 monorepo 結構以平衡負載。副作用：每個跨服務呼叫移除網路跳躍後，延遲從 ~100ms 降至 10ms 以內。
...
主要貢獻：
- **為旗艦產品重構 9 個後端 API 的微服務架構**：從單體架構中抽取共用業務邏輯、定義清晰的服務介面，並將資料存取從直接 DB 呼叫遷移到有版本管理的服務合約
...
在中研院 Citi（網路智慧）實驗室工作。研究目標是透過生成逼真的合成 Linux audit log 來擴增 GAN 入侵偵測模型的訓練資料。
...
**語言：** `TypeScript` `Python` `C#` `Go（學習中）` `SQL`

**後端 & 基礎設施：** `ASP.NET Core` `Node.js` `Kubernetes` `Docker` `MongoDB` `PostgreSQL` `Redis` `微服務` `REST APIs` `CI/CD`

**ML & AI：** `PyTorch` `Transformers (HuggingFace)` `LoRA / PEFT` `Activation Steering` `RAG` `RLHF` `Fine-tuning` `Taiwan-LLaMA` `mT5`

**工具：** `Git` `GitHub Actions` `Unity` `Linux` `Vim` `Grafana`
